...
const FilterGroup: React.FC<Props> = ({ chips, onToggle }) => {
  return (
    <div className="relative">
      <div className="flex overflow-x-auto whitespace-nowrap gap-3 scrollbar-hide py-6 px-4">
        {chips.map((chip) => (
          <button
            key={chip.id}
            onClick={() => onToggle(chip.id)}
            className={`rounded-full px-4 py-1.5 text-sm font-medium transition-all duration-300 flex-shrink-0 ${
              chip.active
